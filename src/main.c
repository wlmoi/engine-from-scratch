...
static const u8 PLAYER_HEALTH_HARD = 3;
static const f32 PLAYER_INVULN_TIME = 0.5f;
...
static bool debug_draw_hitboxes = false;
...
static f32 player_invuln_timer = 0;
static u8 player_health = 0;
...
void player_on_hit(Body *self, Body *other, Hit hit) {
	if (other->collision_layer == COLLISION_LAYER_ENEMY) {
        ...
        player_invuln_timer = PLAYER_INVULN_TIME;
        audio_sound_play(SOUND_HURT);
...
void spawn_enemy(bool is_small, bool is_enraged, bool is_flipped) {
    ...
    entity->health = (u8)(is_small ? HEALTH_ENEMY_SMALL : HEALTH_ENEMY_LARGE);
}
...
void reset(void) {
    ...
    if (global.config.is_hard_mode) {
        player_health = PLAYER_HEALTH_HARD;
    } else {
        player_health = 255;
    }
...
int main(int argc, char *argv[]) {
	...
	while (!should_quit) {
		...
		if (player_invuln_timer > 0) {
			player_invuln_timer -= global.time.delta;
		}
